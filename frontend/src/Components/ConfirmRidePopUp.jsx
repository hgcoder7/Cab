            ...
            <div className="mt-6 w-full" >
                <form onSubmit={(e)=>{submitHandler(e)}}>
                    <input value={otp} onChange={(e)=>setOtp(e.target.value)} type="text" className='bg-[#eee] px-6 py-4 font-mono text-lg rounded-lg w-full mt-3' placeholder='Enter OTP'/>
                        ...
                        }} className='w-full p-3 mt-5 text-lg flex justify-center bg-green-600 text-white font-semibold rounded-lg'>Confirm</Link>
                        <button onClick={()=>{
                        props.setConfirmRidePopupPanel(false)
                        props.setRidePopupPanel(false)}} 
                        className='w-full p-3 mt-2 text-lg bg-red-600 text-white font-semibold rounded-lg'>Cancel</button>
                </form>
            </div>
